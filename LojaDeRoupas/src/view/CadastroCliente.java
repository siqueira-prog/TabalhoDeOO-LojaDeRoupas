...
public class CadastroCliente implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (src == cadastrar) {
			...
			// CPF
			String cpfString = fieldNome.getText();
			...
			// DDD
			String DDDString = fieldNome.getText();
			int DDDInt = Integer.parseInt(DDDString);

			// Telefone
			String TeleString = fieldNome.getText();
			int TeleInt = Integer.parseInt(TeleString);

			// Objeto Telefone
			Telefone telefone = new Telefone(20, DDDInt, TeleInt);

			// Senha
			String senhafield = senha.getText();
//			String comfirmSenhafield = confirmSenha.getText();

			// ENDERECO

			// Logradouro
			String logradouro = fieldLogradouro.getText();
			String numString = fieldNumeroEnd.getText();
			...
			// Obejto Endereco
			Endereco endereco = new Endereco("20", cep, logradouro, numInt, bairro, cidade, estado);

			// Cartão
			String bandeira = fieldBand.getText();
			String numcart = fieldNumCart.getText();

			String codseg = fieldCodSeg.getText();
			String datanasc = fieldDataNasc.getText();
			String mesvencString = fieldMesVenc.getText();
			int mesvenc = Integer.parseInt(mesvencString);
			String anovencString = fieldAnoVenc.getText();
			int anovenc = Integer.parseInt(anovencString);

			// Obejto Cartão
			CartaoDeCredito cartao = new CartaoDeCredito(bandeira, numcart, codseg, datanasc, mesvenc, anovenc);

			ArrayList<String> cadastrocliente = new ArrayList<String>();
			...
			CartaoDeCredito cdc1[] = { cartao };

			Cliente cliente = new Cliente(0, null, null, null, tel, end0, cdc0, 0);

			cliente.cadastrar(cadastrocliente, cpfInt, nome, email, senhafield, telefone, end1, cdc1);

			System.out.print("Sei lá");
